...
def prepare(source_path, output, target):
    ...
    protocol=dict(format='hide-seek-expanded-layouts-v3',physicsSHA256=parent['provenance']['physicsSHA256'],
    ...
    for name in ['train_scaled.py','migrate_environment.py']:copy_immutable(ROOT/'training_v3'/name,prepared/'controller-source'/name)
...
def main(options):
    ...
    def evaluate(path,steps):
        ...
        if not (destination/'evaluation.json').exists():
            command=[sys.executable,str(ROOT/'training_v3/evaluate_saved.py'),
