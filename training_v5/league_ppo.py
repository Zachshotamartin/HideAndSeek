...
def actor_update(model, optimizer, batch, advantages, current_rows, role,
                 ...
                 entropy_weight=.005, kl_limit=.008, burn_in=0):
    ...
    for _ in range(epochs):
        ...
        for offset in range(0, len(ordering), sequence_batch):
            ...
            if not any(bool(active[begin + inner, env].any()) for inner in range(sequence_length)):
                continue
            warm_start=(begin-burn_in).clamp(min=0)
            memory = memories[warm_start, env].detach().clone()
            with torch.no_grad():
                for offset in range(burn_in):
                    warm_step=warm_start+offset
                    live=warm_step<begin
                    if live.any():
                        ids=warm_step[live]; ee=env[live]
                        mm=memory[live]*(1-starts[ids,ee,None])
                        memory[live]=model(observations[ids,ee],mm)[-1]
            loss = 0
            kl_sum = 0
            active_count = 0
