...
class InteractionTests(unittest.TestCase):
    ...
    def test_exclusive_grab_release_and_lock(self):
        e=self.env();a=np.zeros((2,6));a[:,3]=1;e._tools(a)
        self.assertEqual(sum(i>=0 for i in e.grips),1)
        a[0,4]=1;e._tools(a)
        self.assertEqual(e.grips,[-1,-1]);self.assertEqual(e.locks[0],0)
        e._tools(a);self.assertEqual(e.grips,[-1,-1])
        a[0,4]=0;e._tools(a);e._tools(a)
        self.assertEqual(sum(i>=0 for i in e.grips),1)
        a[:]=0;e._tools(a);self.assertEqual(e.grips,[-1,-1])
    def test_jump_grounded_and_height(self):
        e=self.env();a=np.zeros((2,6));e.step(a);a[0,5]=1;e.step(a)
        self.assertEqual(e.jump_events[0],1);peak=e.data.qpos[2]
        for _ in range(5):
            e.step(a);peak=max(peak,e.data.qpos[2])
        self.assertEqual(e.jump_events[0],1)
        self.assertGreater(peak-.25,.7);self.assertLess(peak-.25,.81)
    def test_jump_from_prop_has_wall_safe_apex(self):
        e=self.env();e.data.qpos[:3]=[3.62,3,1.003];mujoco.mj_forward(e.model,e.data)
        a=np.zeros((2,6));e.step(a);a[0,5]=1;peak=e.data.qpos[2]
        for _ in range(12):e.step(a);peak=max(peak,e.data.qpos[2])
        self.assertGreater(e.jump_events[0],0);self.assertLess(peak-.25,2.05)
    def test_can_land_on_box(self):
        e=self.env();e.data.qpos[0]=2.6;mujoco.mj_forward(e.model,e.data)
        a=np.zeros((2,6));e.step(a);a[0,0]=1;a[0,5]=1
        for t in range(15):
            e.step(a);a[0,5]=0
            if t==6:a[0,0]=0
    ...
    def test_jump_does_not_cross_wall(self):
        e=self.env();e.arena['objects']=[];e.arena['walls'].append(dict(position=[3.62,3,1.1],size=[.18,5,2.2],yaw=0));e.reset(arena=e.arena)
        a=np.zeros((2,6));a[0,0]=1;a[0,5]=1
        for _ in range(120):e.step(a)
    ...
    def test_migration_and_ppo(self):
        ...
        result=actor_update(models[0],torch.optim.Adam(models[0].parameters(),lr=1e-4),(ob[None],mem[0][None],torch.ones(1,2),raw[None],logp[None]),torch.tensor([[1.,-1.]]),torch.ones(1,2),0,epochs=1,sequence_length=1,sequence_batch=2)
