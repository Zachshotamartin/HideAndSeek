...
- **Milestone**: at 52,428,800 interactions the candidate seeker should see the hider in at least half of the evaluation rounds with no statistically clear regression in either role. The result is recorded in `STATUS.json` under `gates` and the run continues to its budget either way; a missed milestone is evidence for review, not a halt.
