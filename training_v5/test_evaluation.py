...
from persistent_evaluate import episode, BLACKOUT_STEPS
 ...
 def test_variable_time_memory_and_blackout_interventions(self):
  with tempfile.TemporaryDirectory() as folder:
   ...
   for mode in ['learned','no-seeker-memory','seeker-blackout']:
    ...
    if mode!='seeker-blackout':self.assertEqual(r['blackoutTicks'],0)
   with self.assertRaises(ValueError):episode(models,1750500000,'open','learned',arena_config=dict(size=8,walls=3))
