...
def sample(model, physical, memory, buttons, generator):
    ...
    tape = generator.random(10)
    action = np.zeros(6, np.float32)
    for axis in range(4):
        noise = math.sqrt(-2 * math.log(max(1e-12, tape[axis * 2]))) * math.cos(2 * math.pi * tape[axis * 2 + 1])
        action[axis if axis<3 else 5] = math.tanh(float(normal.mean[0, axis]) + float(normal.scale[0, axis]) * noise)
    if persistent:
...
def episode(models, seed, scenario, mode, trace=False, arena_config=None):
    ...
    if arena_config is not None:
        if set(arena_config)-(set(configuration)|{'prep','play'}):raise ValueError('Only declared geometry and episode durations may vary in evaluation')
        ...
        for tick in range(env.prep+env.play):
            ...
            for role in range(2):
                ...
                histories[role].append(dict(grip=env.grips[role],buttons=buttons[role].tolist(),commands=commands[role],
                    ...
                    seesOpponent=bool(env.seen[role,1-role]),opponentDistance=float(np.linalg.norm(env.data.qpos[role*4:role*4+2]-env.data.qpos[(1-role)*4:(1-role)*4+2])),
    ...
    for role, history in enumerate(histories):
        ...
        seen=[x['seesOpponent'] for x in history[env.prep:]]
        gaps=[];gap=None
        for tick,visible in enumerate(seen):
            if tick and seen[tick-1] and not visible:gap=tick
            if visible and gap is not None:gaps.append((tick-gap)*DT);gap=None
        roles.append(dict(reacquisitionSeconds=gaps,unresolvedLostSight=gap is not None,visiblePursuitProgress=sum(max(-1,min(1,history[t-1]['opponentDistance']-history[t]['opponentDistance'])) for t in range(env.prep+1,len(history)) if history[t-1]['seesOpponent']),gripDurationsTicks=grip,requestedHoldDurationsTicks=holding,
