...
def prepare_assets(source, source_path, prepared, seed):
    ...
    initial_record.update(decisions=0, models=[EntityActor(m.hidden_size, m.encoder_size, m.encoder.embedding_size).state_dict()
                                               for m in large])
...
class Controller:
    ...
    def main(self):
        ...
        try:
            ...
            self.save(phase='paused' if self.stopped else 'pilot-complete' if self.args.stop_after_updates else 'completed-awaiting-review')
