...
def actor_update(model,optimizer,batch,advantages,role,epochs=3,sequence_length=32,
    ...
    if observations.shape[-1]!=210:raise ValueError('Actor accepts only the original restricted140 observations')
...
def train(args):
    ...
    with PhysicsEnvPool(configs,workers=args.workers,with_central_state=True) as pool:
        ...
        for update in range(args.updates):
            ...
            actor_buffers=[[torch.zeros(h,n,210),torch.zeros(h,n,64),torch.zeros(h,n),torch.zeros(h,n,5),torch.zeros(h,n)] for _ in range(2)]
