...
class LeagueTests(unittest.TestCase):
    def test_assignment_and_sample_counters_distinguish_current_frozen_and_blind(self):
        ...
        self.assertTrue(.47 < (b==-1).all(1).mean() < .53)
        physical=np.zeros((4000,2,138),np.float32);physical[:2000,1,5]=1
    ...
    def test_all_frozen_batch_skips_adam_momentum_and_never_reads_extreme_logps(self):
        ...
        batch=[torch.randn(4,2,140),torch.randn(4,2,64),torch.zeros(4,2),torch.full((4,2,5),1e20),torch.full((4,2),float('nan'))]
    ...
    def test_historical_episode_prefix_has_no_gradient_path_after_current_reset(self):
        ...
        observed=torch.randn(4,1,140);observed[:,:,5]=1
