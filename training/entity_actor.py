...
FORMAT = 'original-mujoco-entity-policy-pair-v1'
ENTITY = 'object-set-v1'
LEGACY = 'legacy-linear-v1'
FIXED_INDICES = list(range(18)) + list(range(114, 140))
...
class ObjectSetEncoder(nn.Module):
    def __init__(self, encoder_size=96, embedding_size=32):
        ...
        self.fixed = nn.Linear(44, encoder_size)
        ...
        self.query = nn.Linear(44, embedding_size)
    ...
    def features(self, observation):
        if observation.shape[-1] != 140:
            raise ValueError('The actor still requires exactly 140 restricted observations')
        ...
        fixed[..., 11:18] = torch.where(fixed[..., 10:11] > .5,
                                       fixed[..., 11:18], 0)
        objects = observation[..., 18:114].reshape(*observation.shape[:-1], 6, 16)
        visible = objects[..., 0] > .5
    ...
    def forward(self, observation):
        ...
        weights = torch.softmax(logits, -1)[..., :6]
    ...
    def project(self, original):
        if original['encoder.weight'].shape != (96, 140):
        ...
        with torch.no_grad():
            ...
            self.object_linear.copy_(original['encoder.weight'][:, 18:114].reshape(96, 6, 16).mean(1))
...
def export_actor(model):
    return dict(encoderType=ENTITY if isinstance(model, EntityActor) else LEGACY,
        observationSize=140, physicsObservationSize=138, hiddenSize=model.hidden_size, encoderSize=model.encoder_size,
...
def prepare_pair(parent, *, projected, seed=785611):
    ...
    result = dict(format=FORMAT, encoderTypes=[ENTITY if projected else LEGACY] * 2,
        observationSize=140, physicsObservationSize=138,
