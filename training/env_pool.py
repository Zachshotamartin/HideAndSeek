    ...
    try:
        ...
        while True:
            ...
            if operation=='step':
                ...
                if with_central_state:
                    result=(*result,[central_state(env,(env.actions[:,3:]>.5).astype(np.float32)) for env in envs])
...
class PhysicsEnvPool:
    ...
    def step(self, actions):
        ...
        if actions.shape!=(self.count,2,5) or not np.isfinite(actions).all():raise ValueError(f'Expected finite actions [{self.count},2,5]')
