...
def environment_config(generator, index):
    return dict(scenario=['shelter', 'rooms', 'open'][index % 3],
...
def train(args):
    ...
    if original.get('decisions', 0) != 0 or original['observationSize'] != 138:
    ...
    initial_saved = dict(format=FORMAT, observationSize=140, physicsObservationSize=138,
    ...
    if not resumed:
        ...
        torch.save(dict(format=FORMAT,observationSize=140,physicsObservationSize=138,
    ...
    with PhysicsEnvPool(configs, workers=args.workers) as pool:
        ...
        for local_update in range(args.updates):
            ...
            for role in range(2):
                ...
                buffers.append([torch.zeros(h,n,140),torch.zeros(h,n,64),torch.zeros(h,n),
