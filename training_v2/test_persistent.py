...
class PersistentTests(unittest.TestCase):
    def test_uniform_head_and_exact_warm_start(self):
        ...
        original=PhysicalActor(208);pilot=PersistentActor().warm_start(original.state_dict())
        physical=torch.randn(5,208);memory=torch.randn(5,64);obs=torch.cat((physical,torch.zeros(5,2)),-1)
        with torch.no_grad():
        ...
        self.assertEqual(torch.count_nonzero(pilot.encoder.weight[:,208:]).item(),0)
    ...
    def test_actor_critic_see_only_own_button_states_and_reset_is_zero(self):
        physical=np.zeros((3,2,208),np.float32);buttons=np.zeros((3,2,2),np.float32)
        buttons[1,0]=[1,0];obs=augment(physical,buttons)
        self.assertEqual(obs.shape,(3,2,210))
    ...
    def test_deterministic_decisions_do_not_consume_rng(self):
        model=PersistentActor();obs=torch.zeros(1,210);memory=torch.zeros(1,64)
