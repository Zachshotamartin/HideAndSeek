...
def prepare(source_path, output, target):
    ...
    protocol=dict(format='hide-seek-relational-environment-v2',physicsSHA256=parent['provenance']['physicsSHA256'],
        ...
        reward='Visibility-only zero-sum; zero preparation reward; no tool bonuses.',selfPlay='80% current-current, 20% historical; refreshed every 16 updates',assets={},history=[])
    ...
    extra=[dict(seed=1750100000+i,scenario=['shelter','rooms','open'][i%3],arenaConfig=dict(size=8+(i%5),n_boxes=5+i%4,n_ramps=2)) for i in range(24)]
    atomic_json(prepared/'cohort.json',dict(maps=maps+extra))
    ...
    for name in ['train_scaled.py','migrate_environment.py']:copy_immutable(ROOT/'training_v2'/name,prepared/'controller-source'/name)
...
def main(options):
    ...
    args.training_description='Relational attention; 10 object slots; 30 rays; visibility-only reward; refreshed self-play. Explicit v2 environment migration.'
    ...
    def evaluate(path,steps):
        ...
        if not (destination/'evaluation.json').exists():
            command=[sys.executable,str(ROOT/'training_v2/evaluate_saved.py'),
