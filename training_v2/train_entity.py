        ...
        for update in range(first_update,target_updates):
            ...
            for step in range(h):
                ...
                if finished:
                    ...
                    buttons[finished] = 0
                    role_ids[finished] = assign_roles(opponent_generator,len(finished),args.arm,min(8,len(histories)))
                    historical_mask = role_ids[finished] >= 0
                    selected = role_ids[finished].copy(); selected[historical_mask] += max(0,len(histories)-8); role_ids[finished] = selected
            ...
            keep=sorted(set(range(max(0,len(histories)-8),len(histories)))|set(int(x) for x in role_ids.ravel() if x>=0))
            remap={old:new for new,old in enumerate(keep)}
            for role in range(2):
                role_ids[:,role]=np.array([remap[int(x)] if x>=0 else -1 for x in role_ids[:,role]])
            histories=[histories[i] for i in keep];frozen_before=[frozen_before[i] for i in keep]
            ...
            if (update+1)%args.save_every==0 or update+1 in retained or stopping or update+1==target_updates:
                ...
                saved = dict(format=FORMAT,encoderTypes=parent['encoderTypes'],observationSize=210,physicsObservationSize=208,
                    ...
                    leagueModels=[[m.state_dict() for m in pair] for pair in histories],models=[model.state_dict() for model in models],optimizers=[optimizer.state_dict() for optimizer in optimizers],
