...
def generate_arena(seed=1,scenario='shelter',size=8,n_boxes=3,n_ramps=1):
    if scenario not in ('shelter','rooms','open'):raise ValueError('Unknown arena scenario')
    ...
    def wall(x,y,sx,sy):walls.append(dict(position=[x,y,.45],size=[sx,sy,.9],yaw=0))
...
class PhysicsEnv:
    ...
    def reset(self,seed=None,arena=None):
        ...
        self.t=0;self.returns=np.zeros(2);self.hidden=0;self.grips=[-1,-1];self.locks=np.full(self.n_obj,-1);self.lock_events=np.zeros(2,dtype=int);self.grab_events=np.zeros(2,dtype=int);self.path=np.zeros(2);self.collisions=np.zeros(2,dtype=int);self.shielded=0;self.visible=False;self.done=False;self.actions=np.zeros((2,5));self.last_seen=[None,None]
    ...
    def _tools(self,actions):
        for a in range(2):
            ...
            if not self.disable_tools and actions[a,3]>.5:
                if old>=0:chosen=old
                elif candidates:chosen=min(candidates,key=lambda i:self._distance(a,i))
            if old!=chosen:
                if old>=0:self.data.eq_active[a*self.n_obj+old]=False
                if chosen>=0:self._set_weld(a*self.n_obj+chosen,self.agent_bodies[a],self.object_bodies[chosen]);self.grab_events[a]+=1
                self.grips[a]=chosen
            for i in candidates:
    ...
    def step(self,actions):
        ...
        actions=np.asarray(actions,dtype=float).reshape(2,5).copy();actions[:,:3]=np.clip(actions[:,:3],-1,1)
...
if __name__=='__main__':
    ...
    for _ in range(240):e.step(np.zeros((2,5)))
    print(e.info())
