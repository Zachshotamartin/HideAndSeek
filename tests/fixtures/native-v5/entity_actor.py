...
def restore_league(record):
    ...
    distinguish the two supported encoder families unambiguously.
    """
    ...
    if kinds is None:
        ...
        for pair in pairs:
            types=[]
            for state in pair:
                if 'encoder.embedding.0.weight' in state: types.append(ENTITY)
                elif 'encoder.weight' in state: types.append(LEGACY)
                else: raise ValueError('Unknown saved league encoder')
            kinds.append(types)
    if len(kinds)!=len(pairs): raise ValueError('Missing saved league encoder identities')
    return [load_pair(dict(format=FORMAT,models=pair,encoderTypes=types))[0] for pair,types in zip(pairs,kinds)]
