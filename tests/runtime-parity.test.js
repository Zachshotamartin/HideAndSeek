...
const TAG_PARITY='tag-rounds-native-v6';
const near=(a,b,name)=>{
  assert.equal(a.length,b.length,name);
  a.forEach((v,i)=>assert(Number.isFinite(v)&&Math.abs(v-b[i])<=1e-5,`${name}[${i}]: ${v} versus ${b[i]}`));
};
// A v4 fixture comes from training_v4 and records no exploration noise; a v6
// fixture comes from training_v5 and carries the observed AR(1) noise between rows.
function replay(policies,fixture,tag){
  ...
  for(const rollout of fixture.rollouts){
    ...
    for(const row of rollout.rows){
      ...
      if(tag)near(states[row.role].noise,row.noiseIn,'noiseIn');
      let used=0;
      ...
      if(tag){near(result.noise,row.noiseOut,'noiseOut');if(result.noise.some(v=>v!==0))carried++;}
  ...
  return {rows,blind,resets,carried};
}
for(const entry of manifest.checkpoints)test(`${entry.id} policy matches independent PyTorch recurrence and sampled jump/tool decisions`,()=>{
  ...
  const tag=entry.parityFormat===TAG_PARITY;
  const sourceRoot=tag?'training_v5/':'training_v4/';
  for(const [file,sha]of Object.entries(fixture.sources))assert.equal(hash(read(sourceRoot+file)),sha,`Native fixture source changed: ${file}`);
  const policies=createPolicyControllers(JSON.parse(modelBytes));
  const {rows,blind,resets,carried}=replay(policies,fixture,tag);
  ...
  if(tag)assert(carried>200,'sampled tag-round rows carry exploration noise into the next decision');
});
